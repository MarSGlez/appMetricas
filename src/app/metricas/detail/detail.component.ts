  ...
  constructor(private route: ActivatedRoute,private file: File, private filePath: FilePath, private http: HttpClient, private db: DatabaseService) { 
   ...
   ngOnInit() {
    if (this.destinatario  !== null) {
       ...
       this.db.getMetricasId(this.destinatario).then(  (querySnapshot) => {
        this.metricasD = [];
        querySnapshot.forEach(async (doc) => {
          const item = new Metricas(doc.data());
          this.metricasD.push(item);
          
          /*try {
            const filePath = this.metricasD[0].ruta; // Reemplaza con la ruta correcta del archivo
          ...
          //this.destinatario = this.metricasD[0].ruta;
        });
      });
